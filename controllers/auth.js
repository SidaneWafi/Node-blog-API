...
const signup = async (req, res, next) => {
    try {
        ...
        if (isEmailExist) {
            return res.status(400).json({ code: 400, status: false, message: "Email already exists" });
        }
...
const signin = async (req, res, next) => {
    try {
        ...
        res.status(200).json({ code: 200, status: true, message: "User sign-in successful", data: { token } });
    } catch (error) {
        next(error);
    }
};
...
const verifyCode = async (req, res, next) => {
    try {
        ...
        if (user.isVerified) {
            return res.status(400).json({ code: 400, status: false, message: "User is already verified" });
        }
...
const verifyUser = async (req, res, next) => {
    try {
        ...
        if (user.verificationCode !== code) {
            return res.status(400).json({ code: 400, status: false, message: "Invalid code" });
        }
...
const changePassword = async (req, res, next) => {
    try {
        ...
        if (!match) {
            return res.status(400).json({ code: 400, status: false, message: "Old password does not match" });
        }

        if (oldPassword === newPassword) {
            return res.status(400).json({ code: 400, status: false, message: "You are providing the old password" });
        }
...
const updateProfile= async(req ,res ,next) =>{
    try{
      ...
      await user.save();
      res.status(200).json({code:200 ,status:true,message:"user update successfully",data:{user}});
    }catch(error){
        next(error);
    }
}
module.exports = { signup,
     signin, verifyCode, verifyUser, forgotPasswordCode, recoverPassword, changePassword ,updateProfile};
